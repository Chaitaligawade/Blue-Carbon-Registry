      ...
      <div className="p-4 border-t flex justify-between items-center">
        ...
        </a>
        {
          project.status === "Verified" ?
            <button
              onClick={() => navigate(`/certificate/${project._id}`)}
              className="flex items-center px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg text-sm hover:bg-blue-100"
            >
              <CreditCard className="w-5 h-5 mr-1" />View Certificate
            </button>

            : <button
              onClick={() => onDelete(project._id)}
              className="flex items-center px-3 py-1.5 bg-red-50 text-red-600 rounded-lg text-sm hover:bg-red-100"
            >
